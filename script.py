...
def main():
    ...
    st.session_state.code_prompt = st.text_area(
        ...
        help="Describe the code you want the AI to generate or complete."
    )

    # Settings for input and output options.
    with st.expander("Input Options"):
        with st.container():
            st.session_state.code_input = st.text_input("Input (Stdin)", placeholder="e.g. John Doe, 25", label_visibility='collapsed',value=st.session_state.code_input, help="Provide standard input for code execution.")
            st.session_state.code_output = st.text_input("Output (Stdout)", placeholder="e.g. Hello John Doe!", label_visibility='collapsed',value=st.session_state.code_output, help="Provide expected standard output for code validation.")
            st.session_state.code_fix_instructions = st.text_input("Debug instructions", placeholder="e.g. Fix the null pointer exception.", label_visibility='collapsed',value=st.session_state.code_fix_instructions, help="Instructions for the AI to debug or modify the code.")
    ...
    with st.form('code_controls_form'):
        ...
        with file_name_col:
            code_file = st.text_input("File name", value="", placeholder="e.g. main.py", label_visibility='collapsed', help="Enter the file name to download the generated code.")
