...
def main():
    ...
    st.session_state.code_prompt = st.text_area(
        ...
        help="Enter the prompt for the code you want to generate."
    )

    # Settings for input and output options.
    with st.expander("Input Options"):
        with st.container():
            st.session_state.code_input = st.text_input("Input (Stdin)", placeholder="Input (Stdin)", label_visibility='collapsed',value=st.session_state.code_input, help="Standard input for the code to read, e.g., 'John Doe'")
            st.session_state.code_output = st.text_input("Output (Stdout)", placeholder="Output (Stdout)", label_visibility='collapsed',value=st.session_state.code_output, help="Expected standard output to match, e.g., 'Hello John Doe'")
            st.session_state.code_fix_instructions = st.text_input("Debug instructions", placeholder="Debug instructions", label_visibility='collapsed',value=st.session_state.code_fix_instructions, help="Instructions to help debug the code, e.g., 'Fix the NullPointerException'")
    ...
    with st.form('code_controls_form'):
        ...
        with file_name_col:
            code_file = st.text_input("File name", value="", placeholder="File name", label_visibility='collapsed', help="The name of the file to save the generated code to, e.g., 'script.py'")
