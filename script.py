...
def main():
    ...
    st.session_state.code_prompt = st.text_area(
        ...
        height=130, 
        placeholder="Enter your prompt for code generation. e.g., 'Write a python script that prints Hello World.'" if 'code_prompt' not in st.session_state else "",
        help="Provide a detailed prompt for the code you want to generate.",
    ...
    with st.expander("Input Options"):
        with st.container():
            st.session_state.code_input = st.text_input("Input (Stdin)", placeholder="Input (Stdin) e.g., '10 20'", help="Provide standard input for the code execution.", label_visibility='collapsed',value=st.session_state.code_input)
            st.session_state.code_output = st.text_input("Output (Stdout)", placeholder="Output (Stdout) e.g., '30'", help="Provide expected standard output for the code execution.", label_visibility='collapsed',value=st.session_state.code_output)
            st.session_state.code_fix_instructions = st.text_input("Debug instructions", placeholder="Debug instructions e.g., 'Fix the NullPointerException'", help="Provide instructions for debugging the code.", label_visibility='collapsed',value=st.session_state.code_fix_instructions)
    ...
    with st.form('code_controls_form'):
        ...
        with file_name_col:
            code_file = st.text_input("File name", value="", placeholder="File name e.g., 'script.py'", help="Enter the file name to save the code.", label_visibility='collapsed')
