...
def main():
    ...
    st.session_state.code_prompt = st.text_area(
        ...
        help="Enter the detailed description of the code you want to generate."
    )

    # Settings for input and output options.
    with st.expander("Input Options"):
        with st.container():
            st.session_state.code_input = st.text_input("Input (Stdin)", placeholder="e.g. Test input for your code", label_visibility='collapsed',value=st.session_state.code_input, help="Standard input (stdin) for testing your executed code.")
            st.session_state.code_output = st.text_input("Output (Stdout)", placeholder="e.g. Expected test output", label_visibility='collapsed',value=st.session_state.code_output, help="Expected standard output (stdout) for verification.")
            st.session_state.code_fix_instructions = st.text_input("Debug instructions", placeholder="e.g. Fix the indentation error", label_visibility='collapsed',value=st.session_state.code_fix_instructions, help="Provide instructions on what needs to be fixed or debugged in the code.")
    ...
    with st.form('code_controls_form'):
        ...
        with file_name_col:
            code_file = st.text_input("File name", value="", placeholder="e.g. main.py", label_visibility='collapsed', help="Provide a name for the file to be downloaded.")
