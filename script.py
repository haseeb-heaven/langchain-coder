...
def main():
    ...
    st.session_state.code_prompt = st.text_area(
        ...
        height=130, 
        placeholder="e.g. Write a Python function to read a CSV file" if 'code_prompt' not in st.session_state else "",
        help="Provide detailed instructions or the codebase context for the AI to generate or complete code.",
    ...
    with st.expander("Input Options"):
        with st.container():
            st.session_state.code_input = st.text_input("Input (Stdin)", placeholder="e.g. 5 10 (Inputs separated by space)", help="Standard input for the compiled code", label_visibility='collapsed',value=st.session_state.code_input)
            st.session_state.code_output = st.text_input("Output (Stdout)", placeholder="e.g. 15 (Expected output)", help="Expected standard output to verify", label_visibility='collapsed',value=st.session_state.code_output)
            st.session_state.code_fix_instructions = st.text_input("Debug instructions", placeholder="e.g. Fix the index out of bounds error", help="Instructions for the AI to debug or fix the code", label_visibility='collapsed',value=st.session_state.code_fix_instructions)
    ...
    with st.form('code_controls_form'):
        ...
        with file_name_col:
            code_file = st.text_input("File name", value="", placeholder="e.g. main.py", help="Name of the file to save the downloaded code.", label_visibility='collapsed')
